...
class ReadProductsTable extends Readable {
  constructor(options) {
    ...
    this.count = 0;
    this.arrayOfDates = this.getDates(
      new Date("December 17, 2005"),
  ...
  _read(size) {
    ...
    let dateAdded = this.arrayOfDates[
      randomNumberInt(0, this.arrayOfDates.length)
    ]; // add random date generator here
    ...
    let product_image = `https://picsum.photos/300/300/?image=${Math.floor(
      Math.random() * 1000
    ) + 1}`;
